...
def fetch_yahooquery(stock_symbols):
    """
    Fetches current stock prices using yahooquery and rounds them to 2 decimal places.
    """
    prices = {}
    tickers = Ticker(stock_symbols)
    summary = tickers.summary_detail

    for symbol in stock_symbols:
        try:
            info = summary.get(symbol.lower())
...
if __name__ == "__main__":
    file_path = Path("/content/sample_data/Final_P&L_auto.xlsx")  # Actual file path
    column_to_update = "F"
    start_row = 2
    stock_name_to_scrip = {
    ...
    print("Fetching live data from Yahoo Query...")
    stock_prices = fetch_yahooquery(scrips_to_fetch)
